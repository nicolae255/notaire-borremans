(function () {
  ...
  style.textContent = `
    #cb-bubble {
      ...
      box-shadow: 0 4px 20px rgba(0,180,204,0.4) !important;
      transition: background 0.2s, transform 0.2s !important; z-index: 99999 !important;
    }
    ...
    #cb-teaser {
      ...
      background: #fff; color: #1a2b2e; padding: 0.75rem 1rem;
      border-radius: 12px 12px 4px 12px;
      box-shadow: 0 4px 20px rgba(0,0,0,0.13);
      font-size: 0.875rem; font-family: 'Inter', sans-serif; line-height: 1.4;
      max-width: 220px; z-index: 99998; cursor: pointer;
    ...
    .cb-msg-av { width: 28px; height: 28px; border-radius: 50%; background: #00B4CC; color: #fff; display: flex; align-items: center; justify-content: center; font-size: 0.62rem; font-weight: 700; flex-shrink: 0; font-family: 'Inter', sans-serif; }
    .cb-msg-bbl { max-width: 78%; padding: 0.65rem 0.9rem; border-radius: 14px 14px 14px 4px; font-size: 0.845rem; line-height: 1.55; background: #fff; color: #1a2b2e; box-shadow: 0 1px 4px rgba(0,0,0,0.07); font-family: 'Inter', sans-serif; }
    ...
    .cb-dots span { width: 6px; height: 6px; background: #9bbfc4; border-radius: 50%; animation: cb-blink 1.2s infinite; }
    .cb-dots span:nth-child(2){animation-delay:0.2s;} .cb-dots span:nth-child(3){animation-delay:0.4s;}
    ...
    .cb-step-email input { flex: 1; border: 1px solid #c8dde1; border-radius: 8px; padding: 0.6rem 0.85rem; font-size: 0.845rem; font-family: 'Inter', sans-serif; color: #1a2b2e; outline: none; transition: border-color 0.15s; }
    ...
    .cb-step-msg textarea { border: 1px solid #c8dde1; border-radius: 8px; padding: 0.6rem 0.85rem; font-size: 0.845rem; font-family: 'Inter', sans-serif; color: #1a2b2e; resize: none; outline: none; line-height: 1.5; height: 80px; transition: border-color 0.15s; }
    ...
    .cb-btn { background: #00B4CC; color: #fff; border: none; border-radius: 8px; padding: 0.55rem 1rem; font-size: 0.78rem; font-weight: 700; letter-spacing: 0.06em; text-transform: uppercase; cursor: pointer; display: flex; align-items: center; gap: 0.4rem; transition: background 0.15s; font-family: 'Inter', sans-serif; }
    .cb-btn:hover { background: #009db3; }
    .cb-quick { display: flex; flex-wrap: wrap; gap: 0.4rem; margin-top: 0.5rem; }
    .cb-chip { font-size: 0.72rem; font-weight: 500; padding: 0.3rem 0.65rem; border: 1px solid #c8dde1; border-radius: 20px; cursor: pointer; background: #fff; color: #2d6b76; transition: all 0.15s; white-space: nowrap; font-family: 'Inter', sans-serif; }
    ...
    .cb-success-icon { width: 44px; height: 44px; border-radius: 50%; background: #e8f9ee; color: #22c55e; display: flex; align-items: center; justify-content: center; margin: 0 auto 0.75rem; }
    ...
    @media (max-width: 420px) { #cb-window{width:calc(100vw - 2rem);right:1rem;} #cb-bubble{right:1rem !important;bottom:1rem !important;} }
  `;
  ...
  wrap.innerHTML = `
    ...
    <button id="cb-bubble" aria-label="Ouvrir le chat" type="button">
      ...
      <svg class="cb-ico-close" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" style="display:none"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
    </button>
  ...
  function init() {
    ...
    function scrollBot() { var el = document.getElementById('cb-msgs'); if (el) el.scrollTop = el.scrollHeight; }
    ...
    function renderEmailStep() {
      setFooter(
        '<div class="cb-step-email">' +
          ...
          '<button class="cb-btn" id="cb-email-btn" type="button">' +
    ...
    function renderMsgStep() {
      ...
      var chips = topics.map(function (t) { return '<button class="cb-chip" type="button" data-topic="' + t + '">' + t + '</button>'; }).join('');
      setFooter(
        '<div class="cb-step-msg">' +
          '<div class="cb-quick">' + chips + '</div>' +
          '<textarea id="cb-msg-input" placeholder="Décrivez votre demande…"></textarea>' +
          '<div class="cb-step-msg-row"><button class="cb-btn" id="cb-send-btn" type="button">' +
            '<svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M22 2L11 13"/><path d="M22 2L15 22 11 13 2 9l20-7z"/></svg>&nbsp;Envoyer' +
          '</button></div>' +
        '</div>'
      );
    ...
    function sendEmail(email, subject, body) {
      if (window.emailjs) {
        emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, { from_email: email, sujet: subject, message: body })
          .catch(function (err) { console.error('EmailJS:', err); });
      } else {
        var a = document.createElement('a');
        a.href = 'mailto:' + OFFICE_EMAIL + '?subject=' + encodeURIComponent('Chat — ' + subject) + '&body=' + encodeURIComponent('De : ' + email + '\n\n' + body);
